...
def main(input_file, output_file, mode, similarity_type):
    # Read input TSV into a DataFrame
    df = pd.read_csv(input_file, sep='\t', header=0)

    class_column = [col for col in df.columns if col.endswith('class')][0]
    non_B_rows = df[df[class_column] != 'B']

    # Extract amino acid columns and convert them to a numpy array
    amino_acid_columns = df.columns[:-3]
    data_matrix = df[amino_acid_columns].values
    non_B_data_matrix = non_B_rows[amino_acid_columns].values

    # Initialize a matrix to store the similarities
    similarity_matrix = csr_matrix((len(df), len(non_B_rows)))


    # Create a dictionary to map column indices to unique identifiers
    column_lookup = {i: non_B_rows.iloc[i]['uniprot_id'] + '_' + non_B_rows.iloc[i]['variant'] for i in range(len(non_B_rows))}

    # Iterate over each row and calculate similarity
    for idx in range(len(df)):
        if mode == 'same_ref' or mode == 'same_sub':
            non_B_rows_out = filter_non_B_rows(df, mode, idx)
            non_B_data_matrix = non_B_rows_out[amino_acid_columns].values

        target_vector = data_matrix[idx, :]

        # Check if non_B_data_matrix has at least one sample before calculating similarity
        if len(non_B_data_matrix) > 0:
            for i, row in enumerate(non_B_data_matrix):
                if not (np.isclose(target_vector, row, atol=1e-5).all() or (np.all(target_vector == 0) or np.all(row == 0))):
                    ...
                    # Use the column lookup to get the appropriate column index for the current unique_id
                    column_idx = [k for k, v in column_lookup.items() if v == unique_id][0]

                    # Populate the similarity_matrix with the calculated score
    ...
    output_df = pd.DataFrame(similarity_matrix.toarray(), index=df['uniprot_id'] + '_' + df['variant'],
                         columns=non_B_rows['uniprot_id'] + '_' + non_B_rows['variant'])
...
if __name__ == "__main__":
    ...
    main(args.input_file, args.output_file, args.mode, args.similarity_type)
